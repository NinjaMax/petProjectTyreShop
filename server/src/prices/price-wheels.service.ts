...
export class PriceWheelsService {
  constructor(
    @InjectModel(PriceWheels) private priceWheelsRepository: typeof PriceWheels,
  ...
  async createPriceWheels(createPriceDto: CreatePriceDto) {
    try {
      ...
      if (wheel) {
        const priceCreate = await this.priceWheelsRepository.create(
          createPriceDto,
        );
  ...
  async createPriceWheelsFromPrice(
    ...
    try {
      const [priceByWheelId, created] =
        await this.priceWheelsRepository.findOrCreate({
      ...
      throw new HttpException(
        `${error.message}` + ' Data is incorrect or Not Found',
        HttpStatus.NOT_FOUND,
      );
    }
  }
  ...
  async findAllWheels() {
    try {
      const priceWheelsAll = await this.priceWheelsRepository.findAll({
        include: { all: true },
      });
  ...
  async findAdminPriceWheelsById(id_wheel: number) {
    try {
      const wheelsAdminPriceById = await this.priceWheelsRepository.findAll({
        where: { id_wheel: id_wheel },
        include: { all: true },
      });
      return wheelsAdminPriceById;
    } catch {
      throw new HttpException(
        'Data is incorrect or Not Found',
        HttpStatus.NOT_FOUND,
  ...
  async updatePriceWheels(updatePriceDto: UpdatePriceTyresDto) {
    try {
      const wheelsPriceId = await this.priceWheelsRepository.findByPk(
        updatePriceDto.id_wheel,
        { include: { all: true } },
      );
      if (wheelsPriceId) {
        ...
        const updatePriceWheels = await this.priceWheelsRepository.findByPk(
          updatePriceDto.id_wheel,
          { include: { all: true } },
        );
  ...
  async removePriceWheels(getPriceDto: GetPriceDto) {
    try {
      const removePriceWheels = await this.priceWheelsRepository.destroy({
        where: { id_wheel: getPriceDto.id_wheel },
      });
