...
export class PriceTyresService {
  constructor(
    @InjectModel(PriceTyres) private priceTyresRepository: typeof PriceTyres,
  ...
  async createPriceTyres(createPriceDto: CreatePriceDto) {
    try {
      ...
      if (tyres) {
        const priceCreate = await this.priceTyresRepository.create(
          createPriceDto,
        );
  ...
  async createPriceTyresFromPrice(
    ...
    try {
      const [priceTyreById, created] =
        await this.priceTyresRepository.findOrCreate({
  ...
  async findAllTyres() {
    try {
      const priceTyresAll = await this.priceTyresRepository.findAll({
        include: { all: true },
      });
  ...
  async findAdminPriceTyresById(id_tyre: number) {
    try {
      const tyresAdminById = await this.priceTyresRepository.findAll({
        where: { id_tyre: id_tyre },
        include: { all: true },
      });

      return tyresAdminById;
    } catch {
      throw new HttpException(
        'Data is incorrect or Not Found',
        HttpStatus.NOT_FOUND,
  ...
  async updatePriceTyres(updatePriceDto: UpdatePriceTyresDto) {
    try {
      const tyresId = await this.priceTyresRepository.findByPk(
        updatePriceDto.id_tyre,
        { include: { all: true } },
      );
      ...
      if (tyresId) {
        ...
        const updatePriceTyres = await this.priceTyresRepository.findByPk(
          updatePriceDto.id_tyre,
          { include: { all: true } },
        );
  ...
  async removePrice(getPriceDto: GetPriceDto) {
    try {
      const removePrice = await this.priceTyresRepository.destroy({
        where: { id_tyre: getPriceDto.id_tyre },
      });
