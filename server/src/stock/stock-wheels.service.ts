...
export class StockWheelsService {
  constructor(
    @InjectModel(StockWheels) private stockWheelsRepository: typeof StockWheels,
  ...
  async createStockWheel(createStockDto: CreateStockDto) {
    try {
      ...
      if (wheel) {
        const stockCreate = await this.stockWheelsRepository.create(
          createStockDto,
        );
  ...
  async createStockWheelFromPrice(
    id: number,
    stock: number,
    id_supplier: number,
    update_date: Date,
    ...
    try {
      const [WheelStock, created] =
        await this.stockWheelsRepository.findOrCreate({
  ...
  async findAllStock() {
    try {
      const stockAllWheels = await this.stockWheelsRepository.findAll({
        include: { all: true },
      });
  ...
  async findAdminStockWheelByIdWheel(id_wheel: number) {
    try {
      const stockAdminWheelById = await this.stockWheelsRepository.findAll({
        where: { id_wheel: id_wheel },
        include: { all: true },
      });

      return stockAdminWheelById;
    } catch {
      throw new HttpException(
        'Data is incorrect or Not Found',
        HttpStatus.NOT_FOUND,
  ...
  async findStockWheelByIdForSale(id_wheel: number) {
    try {
      const stockWheelId = await this.stockWheelsRepository.findByPk(id_wheel, {
        include: { all: true },
      });
  ...
  async updateStockWheel(updateStockDto: UpdateStockDto) {
    try {
      const wheelIdUpdate = await this.stockWheelsRepository.findByPk(
        updateStockDto.id_wheel,
        { include: { all: true } },
      );
      ...
      if (wheelIdUpdate) {
        ...
        const updateStockWheel = await this.stockWheelsRepository.findByPk(
          updateStockDto.id_wheel,
          { include: { all: true } },
        );
  ...
  async removeStockWheel(getStockDto: GetStockDto) {
    try {
      const removeStockWheel = await this.stockWheelsRepository.destroy({
        where: { id_wheel: getStockDto.id_wheel },
      });
