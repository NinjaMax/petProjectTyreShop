...
export class SalesService {
  ...
  async addGoodsSale(createSaleDto: CreateSaleDto) {
    try {
      ...
      const findSalesStorage =
        await this.salesStorageService.findAllSalesStorageByOrd(createSaleDto);
      const contractCustomer = await this.contractService.findContractById(
        createSaleDto,
      );
      ...
      if (findSale || findSalesStorage) {
        for (let i = 0; i < findSalesStorage.length; i++) {
          const tyreStock =
            await this.stockTyresService.findStockTyreByIdToAddStock(
              findSalesStorage[i].id,
              findSalesStorage[i].id_supplier,
            );
          const wheelStock =
          ...
          await contractCustomer.decrement('balance', {
            by: findSalesStorage[i].total,
          });
          await contractCustomer.increment('bonus', {
            by: findSalesStorage[i].total * 0.01
          });
          await contractCustomer.reload();
      
          if (tyreStock) {
            await tyreStock.decrement(['stock', 'reserve'], {
              by: createSaleDto.quantity,
            });
            await tyreStock.reload();  
          }
          if (wheelStock) {
            await wheelStock.decrement(['stock', 'reserve'], {
              by: createSaleDto.quantity,
            });
            await wheelStock.reload();
          }
          if (batteryStock) {
            await batteryStock.decrement(['stock', 'reserve'], {
              by: createSaleDto.quantity,
            });
            await batteryStock.reload();
          }
          if (oilStock) {
            await oilStock.decrement(['stock', 'reserve'], {
              by: createSaleDto.quantity,
            });
        ...
        if (findSale.bonus_decrease) {
          // await contractCustomer.increment('balance', {
          //   by: findSale.bonus_decrease,
          // });
          await contractCustomer.decrement('bonus', {
            by: findSale.bonus_decrease,
          });
        }
        return findSale;
  ...
  async findAllSales() {
    try {
      const saleAll = await this.salesRepository.findAll({
        include: { all: true },
      });
      return saleAll;
  ...
  async findSaleById(getSaleDto: GetSaleDto) {
    try {
      const saleById = await this.salesRepository.findByPk(getSaleDto.id_sale, {
        include: { all: true },
      });
      return saleById;
  ...
  async removeSale(getSaleDto: GetSaleDto) {
    try {
      const removeSale = await this.salesRepository.destroy({
