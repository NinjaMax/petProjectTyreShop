...
export class PropsTyreSpeedIndexService {
  constructor(
    @InjectModel(TyreSpeedIndex)
    private tyreSpeedIndexRepository: typeof TyreSpeedIndex,
  ...
  async createTyreSpeedIndexFromPrice(
    id: number,
    speed_index: string,
    speed_index_with_desc: string,
  ) {
    try {
      const [tyreSpeedIndex, created] =
        await this.tyreSpeedIndexRepository.findOrCreate({
  ...
  async findAllTyreSpeedIndex() {
    try {
      const tyreAllSpeedIndex = await this.tyreSpeedIndexRepository.findAll();
  ...
  async findTyreSpeedIndexById(getPropertyDto: GetPropertyDto) {
    try {
      const speedIndexId = await this.tyreSpeedIndexRepository.findByPk(
        getPropertyDto.id_speed_index,
        { include: { all: true } },
      );
  ...
  async updateTyreSpeedIndex(updatePropertyDto: UpdatePropertyDto) {
    try {
      const speedIndexTyresId = await this.tyreSpeedIndexRepository.findByPk(
        updatePropertyDto.id_speed_index,
        { include: { all: true } },
      );
  ...
  async removeTyreSpeedIndex(getPropertyDto: GetPropertyDto) {
    try {
      const removeTyreSpeedIndexes =
        await this.tyreSpeedIndexRepository.destroy({
          where: { id_speed_index: getPropertyDto.id_speed_index },
        });
