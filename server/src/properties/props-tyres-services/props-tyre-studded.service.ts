...
export class PropsTyreStuddedService {
  constructor(
    @InjectModel(TyreStudded) private tyreStuddedRepository: typeof TyreStudded,
  ...
  async createTyreStuddedFromPrice(id: number, studded: string) {
    try {
      const [tyreStudded, created] =
        await this.tyreStuddedRepository.findOrCreate({
          where: { studded: studded },
          defaults: { studded: studded },
        });
  ...
  async findAllTyreStudded() {
    try {
      const tyreAllStudded = await this.tyreStuddedRepository.findAll();
  ...
  async findTyreStuddedById(getPropertyDto: GetPropertyDto) {
    try {
      const studdedId = await this.tyreStuddedRepository.findByPk(
        getPropertyDto.id_studded,
        { include: { all: true } },
      );
  ...
  async updateTyreStudded(updatePropertyDto: UpdatePropertyDto) {
    try {
      const studdedTyresId = await this.tyreStuddedRepository.findByPk(
        updatePropertyDto.id_studded,
        { include: { all: true } },
      );

      if (studdedTyresId) {
        const updateStudded = await this.tyreStuddedRepository.update(
          { studded: updatePropertyDto.studded },
          { where: { id_studded: updatePropertyDto.id_studded } },
        );
  ...
  async removeTyreStudded(getPropertyDto: GetPropertyDto) {
    try {
      const removeTyreStuddeds = await this.tyreStuddedRepository.destroy({
        where: { id_studded: getPropertyDto.id_studded },
      });
