...
export class PropsTyreReinforceService {
    ...
    try {
        ...
        if(tyreId && tyreReinforce) {
            ...
            const updateReinforce = await this.tyreReinforceRepository.update({
             reinforce: createPropertyDto.reinforce}, {where: {id_reinforce: tyreReinforce.id_reinforce}});
            ...
            const newTyreReinforce = await this.tyreReinforceRepository.create(createPropertyDto);
            ...
            const tyreReinforce = await this.tyreReinforceRepository.create(createPropertyDto);
  ...
  async findAllTyreReinforce() {
    ...
    try {
      ...
      const tyreAllReinforce = await this.tyreReinforceRepository.findAll({include: {all: true}});
  ...
  async findTyreReinforceById(getPropertyDto: GetPropertyDto) {
    ...
    try {
      ...
      const reinforceId = await this.tyreReinforceRepository.findByPk(getPropertyDto.id_reinforce, {include: {all: true}});
  ...
  async updateTyreReinforce( updatePropertyDto: UpdatePropertyDto) {
    ...
    try {
      ...
      const reinforceTyresId = await this.tyreReinforceRepository.findByPk(updatePropertyDto.id_reinforce, {include: {all: true}});

      if(reinforceTyresId) {

        const updateReinforce = await this.tyreReinforceRepository.update(
        { reinforce: updatePropertyDto.reinforce}, {where: {id_reinforce : updatePropertyDto.id_reinforce}});
