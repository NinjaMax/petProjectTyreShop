...
export class PropsTyreYearService {
  constructor(
    @InjectModel(TyreYear) private tyreYearRepository: typeof TyreYear,
  ...
  async createTyreYear(createPropertyDto: CreatePropertyDto) {
    try {
      ...
      if (tyreId && tyreYear) {
        ...
        const newTyreYear = await this.tyreYearRepository.create(
          createPropertyDto,
        );
        ...
        const tyreYear = await this.tyreYearRepository.create(
          createPropertyDto,
        );
  ...
  async findAllTyreYear() {
    try {
      const tyreAllYear = await this.tyreYearRepository.findAll();
      return tyreAllYear;
    } catch {
      throw new HttpException(
  ...
  async findTyreYearById(getPropertyDto: GetPropertyDto) {
    try {
      const yearId = await this.tyreYearRepository.findByPk(
        getPropertyDto.id_year,
        { include: { all: true } },
      );
  ...
  async updateTyreYear(updatePropertyDto: UpdatePropertyDto) {
    try {
      const yearTyresId = await this.tyreYearRepository.findByPk(
        updatePropertyDto.id_year,
        { include: { all: true } },
      );

      if (yearTyresId) {
        const updateYear = await this.tyreYearRepository.update(
          { manufacture_year: updatePropertyDto.manufacture_year },
          { where: { id_year: updatePropertyDto.id_year } },
        );
      ...
      return new HttpException(
        `Data "id_year" or "year" is incorrect or Not Found`,
        HttpStatus.NOT_FOUND,
      );
    } catch {
      throw new HttpException(
  ...
  async removeTyreYear(getPropertyDto: GetPropertyDto) {
    try {
      const removeTyreYears = await this.tyreYearRepository.destroy({
        where: { id_year: getPropertyDto.id_year },
      });
