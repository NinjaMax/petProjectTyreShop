...
export class PropsTyreSeasonService {
  constructor(
    @InjectModel(TyreSeason) private tyreSeasonRepository: typeof TyreSeason,
  ...
  async createTyreSeasonFromPrice(
    id: number,
    id_season: number,
    season: string,
    season_ua: string,
    ...
    try {
      const [tyreSeason, created] =
        await this.tyreSeasonRepository.findOrCreate({
          where: { season: season },
          defaults: {
            id_season: id_season,
            season: season,
  ...
  async findTyreSeasonById(getPropertyDto: GetPropertyDto) {
    try {
      const seasonId = await this.tyreSeasonRepository.findByPk(
        getPropertyDto.id_season,
        { include: { all: true } },
      );
  ...
  async updateTyreSeason(updatePropertyDto: UpdatePropertyDto) {
    try {
      const seasonTyresId = await this.tyreSeasonRepository.findByPk(
        updatePropertyDto.id_season,
        { include: { all: true } },
      );

      if (seasonTyresId) {
        const updateSeason = await this.tyreSeasonRepository.update(
          { season: updatePropertyDto.season },
          { where: { id_season: updatePropertyDto.id_season } },
        );
  ...
  async removeTyreSeason(getPropertyDto: GetPropertyDto) {
    try {
      const removeTyreSeason = await this.tyreSeasonRepository.destroy({
        where: { id_season: getPropertyDto.id_season },
      });
