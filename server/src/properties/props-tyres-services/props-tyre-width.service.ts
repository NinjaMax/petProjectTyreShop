...
export class PropsTyreWidthService {
  constructor(
    @InjectModel(TyreWidth) private tyreCountryRepository: typeof TyreWidth,
  ...
  async createTyreWidth(createPropertyDto: CreatePropertyDto) {
    try {
      ...
      if (tyreId && tyreWidth) {
        ...
        const tyreWidth = await this.tyreCountryRepository.create(
          createPropertyDto,
        );
        return tyreWidth;
      }
  ...
  async createTyreWidthFromPrice(id: number, width: string) {
    try {
      const [tyreWidth, created] =
        await this.tyreCountryRepository.findOrCreate({
          where: { width: width },
  ...
  async findAllTyreWidth() {
    try {
      const tyreAllWidth = await this.tyreCountryRepository.findAll();
      return tyreAllWidth;
    } catch {
      throw new HttpException(
  ...
  async findTyreWidthById(getPropertyDto: GetPropertyDto) {
    try {
      const widthId = await this.tyreCountryRepository.findByPk(
        getPropertyDto.id_width,
        { include: { all: true } },
      );
      return widthId;
    } catch {
  ...
  async updateTyreWidth(updatePropertyDto: UpdatePropertyDto) {
    try {
      const countryTyresId = await this.tyreCountryRepository.findByPk(
        updatePropertyDto.id_width,
        { include: { all: true } },
      );
      if (countryTyresId) {
        const updateWidth = await this.tyreCountryRepository.update(
          { width: updatePropertyDto.width },
          { where: { id_width: updatePropertyDto.id_width } },
        );
        return updateWidth;
      }
  ...
  async removeTyreWidth(getPropertyDto: GetPropertyDto) {
    try {
      const removeTyreWidths = await this.tyreCountryRepository.destroy({
        where: {
          id_width: getPropertyDto.id_width,
        },
      });
