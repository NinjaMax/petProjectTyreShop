...
export class PropsTyreRunFlatService {
  constructor(
    @InjectModel(TyreRunFlat) private tyreRunFlatRepository: typeof TyreRunFlat,
  ...
  async createTyreRunFlat(createPropertyDto: CreatePropertyDto) {
    try {
      ...
      if (tyreId && tyreRunFlat) {
        ...
        const newTyreRunFlat = await this.tyreRunFlatRepository.create(
          createPropertyDto,
        );
        ...
        const tyreRunFlat = await this.tyreRunFlatRepository.create(
          createPropertyDto,
        );
  ...
  async createTyreRunFlatFromPrice(id: number, run_flat: string) {
    try {
      const [tyreRunFlat, created] =
        await this.tyreRunFlatRepository.findOrCreate({
          where: { run_flat: run_flat },
          defaults: { run_flat: run_flat },
        });
  ...
  async findAllTyreRunFlat() {
    try {
      const tyreAllRunFlat = await this.tyreRunFlatRepository.findAll();
  ...
  async findTyreRunFlatById(getPropertyDto: GetPropertyDto) {
    try {
      const runFlatId = await this.tyreRunFlatRepository.findByPk(
        getPropertyDto.id_run_flat,
        { include: { all: true } },
      );
  ...
  async updateTyreRunFlat(updatePropertyDto: UpdatePropertyDto) {
    try {
      const runFlatTyresId = await this.tyreRunFlatRepository.findByPk(
        updatePropertyDto.id_run_flat,
        { include: { all: true } },
      );

      if (runFlatTyresId) {
        const updateRunFlat = await this.tyreRunFlatRepository.update(
          { run_flat: updatePropertyDto.run_flat },
          { where: { id_run_flat: updatePropertyDto.id_run_flat } },
        );
  ...
  async removeTyreRunFlat(getPropertyDto: GetPropertyDto) {
    try {
      const removeTyreRunFlats = await this.tyreRunFlatRepository.destroy({
        where: { id_run_flat: getPropertyDto.id_run_flat },
      });
