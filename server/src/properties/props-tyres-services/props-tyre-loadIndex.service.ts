...
export class PropsTyreLoadIndexService {
  constructor(
    @InjectModel(TyreLoadIndex)
    private tyreLoadIndexRepository: typeof TyreLoadIndex,
  ...
  async createTyreLoadIndex(createPropertyDto: CreatePropertyDto) {
    try {
      ...
      if (tyreId && tyreLoadIndex) {
        ...
        const newTyreLoadIndex = await this.tyreLoadIndexRepository.create(
          createPropertyDto,
        );
        ...
        const tyreLoadIndex = await this.tyreLoadIndexRepository.create(
          createPropertyDto,
        );
  ...
  async createLoadIndexFromPrice(
    id: number,
    load_index: string,
    load_index_with_desc: string,
  ) {
    try {
      const [tyreLoadIndex, created] =
        await this.tyreLoadIndexRepository.findOrCreate({
  ...
  async findAllTyreLoadIndex() {
    try {
      const tyreAllLoadIndex = await this.tyreLoadIndexRepository.findAll();
  ...
  async findTyreLoadIndexById(getPropertyDto: GetPropertyDto) {
    try {
      const loadIndexId = await this.tyreLoadIndexRepository.findByPk(
        getPropertyDto.id_load_index,
        { include: { all: true } },
      );
  ...
  async updateTyreLoadIndex(updatePropertyDto: UpdatePropertyDto) {
    try {
      const loadIndexTyresId = await this.tyreLoadIndexRepository.findByPk(
        updatePropertyDto.id_load_index,
        { include: { all: true } },
      );

      if (loadIndexTyresId) {
        const updateLoadIndex = await this.tyreLoadIndexRepository.update(
          { load_index: updatePropertyDto.load_index },
          { where: { id_load_index: updatePropertyDto.id_load_index } },
        );
      ...
      return new HttpException(
        `Data "id_load_index" or "load_index" is incorrect or Not Found`,
        HttpStatus.NOT_FOUND,
      );
    } catch {
      throw new HttpException(
  ...
  async removeTyreLoadIndex(getPropertyDto: GetPropertyDto) {
    try {
      const removeTyreLoadIndexes = await this.tyreLoadIndexRepository.destroy({
        where: { id_load_index: getPropertyDto.id_load_index },
      });
