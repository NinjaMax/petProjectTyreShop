...
export class PropsTyreHomologationService {
  constructor(
    @InjectModel(TyreHomologation)
    private tyreHomologationRepository: typeof TyreHomologation,
  ...
  async createTyreHomologation(createPropertyDto: CreatePropertyDto) {
    try {
      ...
      if (tyreId && tyreHomologation) {
        ...
        const newTyreHomologation =
          await this.tyreHomologationRepository.create(createPropertyDto);
        ...
        const tyreHomologation = await this.tyreHomologationRepository.create(
          createPropertyDto,
        );
  ...
  async createTyreHomologationFromPrice(id: number, homologation: string) {
    try {
      const [tyreHomologation, created] =
        await this.tyreHomologationRepository.findOrCreate({
  ...
  async findAllTyreHomologation() {
    try {
      const tyreAllHomologation =
        await this.tyreHomologationRepository.findAll();
  ...
  async findTyreHomologationById(getPropertyDto: GetPropertyDto) {
    try {
      const homologationId = await this.tyreHomologationRepository.findByPk(
        getPropertyDto.id_homologation,
        { include: { all: true } },
      );
  ...
  async updateTyreHomologation(updatePropertyDto: UpdatePropertyDto) {
    try {
      const homologationTyresId =
        await this.tyreHomologationRepository.findByPk(
          updatePropertyDto.id_homologation,
          { include: { all: true } },
        );

      if (homologationTyresId) {
        const updateHomologation = await this.tyreHomologationRepository.update(
          { homologation: updatePropertyDto.homologation },
          { where: { id_homologation: updatePropertyDto.id_homologation } },
        );
  ...
  async removeTyreHomologation(getPropertyDto: GetPropertyDto) {
    try {
      const removeTyreHomologations =
        await this.tyreHomologationRepository.destroy({
          where: { id_homologation: getPropertyDto.id_homologation },
        });
