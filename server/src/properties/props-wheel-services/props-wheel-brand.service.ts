...
export class PropsWheelBrandService {
  constructor(
    @InjectModel(WheelBrand) private wheelBrandRepository: typeof WheelBrand,
  ...
  async createWheelBrand(createPropertyDto: CreatePropertyDto) {
    try {
      ...
      if (wheelId) {
        const wheelBrand = await this.wheelBrandRepository.create(
          createPropertyDto,
        );
        ...
        const newWheelBrand = await this.wheelBrandRepository.create(
          createPropertyDto,
        );
  ...
  async createWheelBrandFromPrice(id: number, brand: string) {
    try {
      const [wheelBrand, created] =
        await this.wheelBrandRepository.findOrCreate({
          where: { brand: brand },
  ...
  async findAllWheelBrand() {
    try {
      const wheelAllBrand = await this.wheelBrandRepository.findAll({
        include: { all: true },
      });
  ...
  async findWheelBrandById(getPropertyDto: GetPropertyDto) {
    try {
      const brandId = await this.wheelBrandRepository.findByPk(
  ...
  async findWheelBrandByName(brand: string) {
    try {
      const brandByName = await this.wheelBrandRepository.findOne({
        where: { brand: brand },
  ...
  async updateWheelBrand(updatePropertyDto: UpdatePropertyDto) {
    try {
      const brandWheelId = await this.wheelBrandRepository.findByPk(
  ...
  async removeWheelBrand(getPropertyDto: GetPropertyDto) {
    try {
      const removeWheelBrands = await this.wheelBrandRepository.destroy({
        where: { id_brand: getPropertyDto.id_brand },
      });
